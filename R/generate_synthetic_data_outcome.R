...
<<<<<<< HEAD
  # Setting 7
    ...
    mu <- -10 - sum(c(2, 2, 3, -1) * cf14) - 2 * (cf5 + cf6) +
      10 * (1 + beta * (0.5 * em1 - 0.8 * em2)) * treat + 0.001 * treat^3
  }

  # Heterogenous outcome
  if (heterogenous_intercept) { # setting 3 and 4
    mu <- mu + 20 * (em1 + 2 * em2)
  }

  # produce outcome Y
  Y <- mu + stats::rnorm(nrow(treat), mean = 0, sd = abs(mu$treat))
  # Y <- mu + stats::rnorm(nrow(treat), mean = 0, sd = outcome_sd)
