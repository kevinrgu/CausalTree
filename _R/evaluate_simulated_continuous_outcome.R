...
evaluate_simulated_continuous_outcome <- function(lambdas, correct_splits, noise.var, num_exposure_cats, gps_spec = 1, em_spec = 1,
  ...
  synth_data <- generate_syn_data_het(
    sample_size = 20000, outcome_type = "continuous",
  ...
  parms.used1 <- list(
    w = matched.exploration.sample$w,
...
>>>>>>> na/main
  ...
  g.h.list1 <- lists1[[2]]
<<<<<<< HEAD
  tree.sizes1 <- sapply(tree.list1, function(t) {
...
<<<<<<< HEAD
    ...
    selected.tree.size1 <- complex.vals1 %>%
      group_by(lambda) %>%
=======
...
<<<<<<< HEAD
  ...
  selected.tree.size <- selected.tree.sizes %>%
    group_by(lambda) %>%
    summarise_at(vars(tree.size), function(s) {
      round(mean(s))
    }) %>%
    ungroup()

  selected.trees <- lapply(selected.tree.size$tree.size, function(s) {
    tree.list1[[which(tree.sizes1 == s)]]
  })
  names(selected.trees) <- lambdas
  est.treatment.effects <- lapply(
    selected.trees,
    function(t) {
      mutate(inference.sample, pred = predict(t, newdata = inference.sample))
    }
  )
  ...
  mse <- sapply(est.treatment.effects, function(est.treatment.effect) {
    mean((est.treatment.effect$pred - true_trt_effects$eff)^2)
  })

  bias <- sapply(est.treatment.effects, function(est.treatment.effect) {
    mean((est.treatment.effect$pred - true_trt_effects$eff))
  })
  ...
  correct.tree.in.sequence <- any(sapply(tree.list1, function(t) {
    list(row.names(t$splits)) %in% correct_splits
  }))
...
for (i in 1:50) {
  ret <- evaluate_simulated_continuous_outcome(
    lambda = lambdas, correct_splits = correct_splits, noise.var = noise.var,
...
selected.tree.data %>%
  ggplot(aes(x = lambda, y = mse, group = lambda)) +
  geom_boxplot() +
...
selected.tree.data %>%
  ggplot(aes(x = lambda, y = bias, group = lambda)) +
  geom_boxplot() +
=======
...
selected.tree.data.combined %>%
  filter(lambda == 3000) %>%
  ggplot(aes(y = bias, group = lambda)) +
  geom_boxplot() +
  facet_wrap(vars(setting), nrow = 1, labeller = labeller(setting = setting.labs)) +
  scale_x_continuous(breaks = lambdas)
...
selected.tree.data %>%
<<<<<<< HEAD
  ggplot(aes(x = lambda, fill = selected.correct.splits, y = ..count.. / 100)) +
...
selected.tree.data %>%
  ggplot(aes(x = lambda, y = numb.noise, group = lambda)) +
  geom_boxplot() +
  scale_x_continuous(breaks = lambdas) +
  labs(y = "Proportion of Simulation trials")
ggsave(paste0(dir_out, "Prop.correct.trees.no.stopping.png"))
...
selected.tree.data %>%
  ...
  ggplot(aes(x = lambda, fill = `Pruned to Correct Size`, y = ..count.. / 100)) +
  geom_histogram(position = "dodge") +
  scale_x_continuous(breaks = lambdas) +
  labs(y = "Proportion of Simulation trials")
...
selected.tree.data %>%
  ggplot(aes(x = lambda, y = floor(tree.size), group = lambda)) +
  geom_boxplot() +
...
selected.tree.data %>%
<<<<<<< HEAD
  ggplot(aes(x = lambda, y = selected.tree.size, group = lambda)) +
  geom_boxplot() +
  scale_x_continuous(breaks = lambdas)
...
selected.tree.data %>%
  ...
  ggplot(aes(x = lambda, fill = `Pruned to Correct Size`)) +
  geom_histogram(position = "dodge") +
  scale_x_continuous(breaks = lambdas)
...
<<<<<<< HEAD
  summarise(
    prop_correct = sum(selected.correct.splits) / n(),
